"""
@file example_MNIST_MLP_ChunkedHypernetwork.py
...
This Hypernetwork uses the chunked version of Hypernetworks where there are multiple embedding vectors
that are passed in and the Hypernetworks outputs a 'chunk' of the main net at a time to reduce parameter counts.
...
class Net(nn.Module):
    def __init__(self):
        ...
        # Specifying the number of embedding vectors to use in the chunked hypernetwork
        ...
        self.embedding = nn.Parameter(torch.randn([self.num_embeddings, 10], requires_grad=True).float().cuda())

        # Chunked hypernetwork, takes the embeddings and outputs partial weight chunks of the main network
        self.hypernet = nn.Sequential(
            nn.Linear(10, 16),
            nn.ReLU(),
            nn.Linear(16, self.total_mainnet_params // self.num_embeddings)
        )
    ...
    def sample_weights(self):
        # Get the main net weight chunked outputs from the hypernetwork and reshape to the weight vector
        weights = self.hypernet(self.embedding)
